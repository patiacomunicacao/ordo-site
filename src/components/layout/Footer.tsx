...
export default async function Footer() {
  ...
  const quickLinks = [
    ...
    { href: "/#como-trabalhamos", label: t("links.howWeWork") },
    { href: "/casos", label: t("links.cases") },
    { href: "/blog", label: t("links.blog") },
    { href: "/#contato", label: t("links.contact") },
  ];

  return (
    <footer className="on-dark text-white" style={{ backgroundColor: "#26123A" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 pb-12 border-b border-white/10">
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              {cfg.instagram && (
                <li className="flex items-start gap-2.5">
                  <span className="text-purple-400 mt-0.5 flex-shrink-0"><InstagramIcon size={15} /></span>
                  <a href={cfg.instagram} target="_blank" rel="noopener noreferrer" className="text-sm text-purple-200 hover:text-white transition-colors">
                    {instagramHandle(cfg.instagram)}
                  </a>
                </li>
              )}
              <li className="flex items-start gap-2.5">
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-purple-400">
          <p>© {new Date().getFullYear()} ORDO Automação. {t("copyright")}</p>
